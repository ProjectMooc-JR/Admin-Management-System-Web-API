    ...
    if (result.isSuccess) {
      res
        ...
        .json({
          ...
          chapterId: result.data.insertId,
        });
    } else {
...
const updateChapterByCourseAndOrderAsync = async (req, res) => {
  ...
  let result = await chapterService.updateChapterByCourseAndOrderAsync(
    courseId,
    chapterOrder,
    chapter
  );
...
const deleteChapterByCourseAndOrderAsync = async (req, res) => {
  try {
    ...
    if (isNaN(courseId) || isNaN(chapterOrder)) {
      console.error(
        `Invalid courseId (${courseId}) or chapterOrder (${chapterOrder})`
      );
    ...
    console.log(
      `Deleting chapter with CourseID: ${courseId}, ChapterOrder: ${chapterOrder}`
    );

    // Call the service layer to delete the chapter
    const result = await chapterService.deleteChapterByCourseAndOrderAsync(
      courseId,
      chapterOrder
    );
    ...
    if (result.isSuccess) {
      console.log(
        `Chapter deleted successfully for CourseID: ${courseId}, ChapterOrder: ${chapterOrder}`
      );
      ...
      return res.sendCommonValue(
        {},
        result.message || "Failed to delete chapter",
        result.status || 400
      );
    }
  } catch (error) {
...
const getChaptersByCourseIdWithPaginationAsync = async (req, res) => {
  ...
  try {
    // 调用修改后的 service 层函数
    const result = await chapterService.getAllChaptersByPaginationAsync(
      courseId,
      page,
      pageSize
    );
