...
const loginAsync = async (req, res) => {
  ...
  try {
    let result = await userservice.getUserbyNameAsync(username);
    if (!result.isSuccess) {
      res.sendCommonValue(null, "login fail username or password not match", 0);
    } else {
      ...
      if (isMatch) {
        ...
        res.sendCommonValue(
          null,
          "login fail username or password not match",
          0
        );
      }
    }
    ...
    res.status(500).sendCommonValue(null, "Server error", 0);
  }
};
